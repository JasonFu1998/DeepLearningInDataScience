...
% Gradient Check
% X_check = train.X(1:10, 1:2);
% Y_check = train.Y(:, 1:2);
% NP.W{1} = NP.W{1}(:, 1:10);
% [P, H] = EvaluateClassifier(X_check, NP);
% ga = ComputeGradients(X_check, Y_check, P, H, NP, hp.lambda);
% NP.use_bn = true;
% gn = ComputeGradsNumSlow(X_check, Y_check, NP, hp.lambda, 1e-5);
% k = size(NP.W, 2);
% W_r = zeros(1, k);
% b_r = zeros(1, k);
% gamma_r = zeros(1, k - 1);
% beta_r = zeros(1, k - 1);
% for i = 1: k
%     W_relative = abs(ga.W{i} - gn.W{i}) ./ max(1e-6, abs(ga.W{i}) + abs(gn.W{i}));
%     W_r(i) = mean(W_relative(:));
%     b_relative = abs(ga.b{i} - gn.b{i}) ./ max(1e-6, abs(ga.b{i}) + abs(gn.b{i}));
%     b_r(i) = mean(b_relative(:));
% end
% for i = 1: k - 1
%     gamma_relative = abs(ga.gamma{i} - gn.gamma{i}) ./ max(1e-6, abs(ga.gamma{i}) + abs(gn.gamma{i}));
%     gamma_r(i) = mean(gamma_relative(:));
%     beta_relative = abs(ga.beta{i} - gn.beta{i}) ./ max(1e-6, abs(ga.beta{i}) + abs(gn.beta{i}));
%     beta_r(i) = mean(beta_relative(:));
% end
...
function [W, b, gamma, beta] = ParameterInitialize(m)
    ...
    for i = 1: k - 1
        ...
        % W{i} = sqrt(2 / (m(i) + m(i + 1))) * randn(m(i + 1), m(i));
        % W{i} = 1e-1 * randn(m(i + 1), m(i));
        b{i} = zeros(m(i + 1), 1);
        gamma{i} = ones(m(i + 1), 1);
        beta{i} = zeros(m(i + 1), 1);
    end
    W{k} = 1 / sqrt(m(k)) * randn(m(k + 1), m(k));
    % W{k} = 1e-1 * randn(m(k + 1), m(k));
    b{k} = zeros(m(k + 1), 1); 
end
